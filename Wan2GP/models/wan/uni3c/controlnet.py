...
Directly ported from Kijai's ComfyUI-WanVideoWrapper implementation.
...
def sageattn_func(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False):
    """
    Fallback to SDPA if SageAttention is not available.
    SageAttention is an optimized attention implementation, but SDPA works fine.
    """
    try:
...
class MaskCamEmbed(nn.Module):
    def __init__(self, controlnet_cfg) -> None:
        ...
        if controlnet_cfg.get("interp", False):
            self.mask_padding = [0, 0, 0, 0, 3, 3]  # left/right/top/bottom/front/back, I2V-interp
        else:
        ...
        self.mask_proj = nn.Sequential(
            nn.Conv3d(add_channels, mid_channels, kernel_size=(4, 8, 8), stride=(4, 8, 8)),
...
class WanControlNet(nn.Module):
    ...
    Ported directly from Kijai's ComfyUI-WanVideoWrapper implementation
        ...
        self.controlnet_blocks = nn.ModuleList(
            [
                SingleAttentionBlock(
                    ...
                    qk_norm="rms_norm_across_heads",
                    attention_mode=controlnet_cfg.get("attention_mode", "sdpa"),
                )
    ...
    def forward(
        ...
        render_mask: Optional[torch.Tensor],
        camera_embedding: Optional[torch.Tensor],
        temb: torch.Tensor,
        ...
        Args:
            render_latent: Guide video latents [B, C, F, H, W] (36 channels: 20 noisy + 16 guide)
            render_mask: Optional mask tensor
            camera_embedding: Optional camera embedding
            temb: Timestep embedding [B, 5120]
            out_device: Device to move outputs to
