...
class WanT2V:
    ...
    def vace_encode_frames_multi(self, frames_list, ref_images_list, masks_list=None, tile_size=0, overlapped_latents=None):
        """
        Encode multiple sets of VACE frames, reference images, and masks.
        
        Args:
            frames_list: List of frame lists, each representing a different VACE stream
            ref_images_list: List of reference image lists for each stream
            masks_list: List of mask lists for each stream (optional)
            tile_size: VAE tile size
            overlapped_latents: Overlapped latents (optional)
            
        Returns:
            List of encoded latents for each VACE stream
        """
        if masks_list is None:
            masks_list = [None] * len(frames_list)
        if ref_images_list is None:
            ref_images_list = [None] * len(frames_list)
            
        encoded_streams = []
        for frames, ref_images, masks in zip(frames_list, ref_images_list, masks_list):
            if frames is not None:
                encoded = self.vace_encode_frames(frames, ref_images, masks, tile_size, overlapped_latents)
                encoded_streams.append(encoded)
            else:
                encoded_streams.append(None)
                
        return encoded_streams

    def vace_encode_masks_multi(self, masks_list, ref_images_list=None):
        """
        Encode multiple sets of VACE masks.
        
        Args:
            masks_list: List of mask lists for each VACE stream
            ref_images_list: List of reference image lists for each stream (optional)
            
        Returns:
            List of encoded masks for each VACE stream
        """
        if ref_images_list is None:
            ref_images_list = [None] * len(masks_list)
            
        encoded_masks = []
        for masks, ref_images in zip(masks_list, ref_images_list):
            if masks is not None:
                encoded = self.vace_encode_masks(masks, ref_images)
    ...
    def vace_latent_multi(self, z_list, m_list):
        """
        Combine encoded frames and masks for multiple VACE streams.
        
        Args:
            z_list: List of encoded frame lists for each stream
            m_list: List of encoded mask lists for each stream
            
        Returns:
            List of combined latents for each VACE stream
        """
        combined_streams = []
        for z, m in zip(z_list, m_list):
            if z is not None and m is not None:
                combined = self.vace_latent(z, m)
                combined_streams.append(combined)
            elif z is not None:
                combined_streams.append(z)
            else:
                combined_streams.append(None)
    ...
    def generate(self,
                ...
                multi_vace_inputs = None,  # List of dicts with 'frames', 'masks', 'ref_images', 'strength', 'start_percent', 'end_percent'
                input_video=None,
                target_camera=None,                  
                context_scale=1.0,
                # VACE step control parameters
                vace_start_percent=0.0,  # Start applying VACE at this percentage of steps (0.0 = from beginning)
                vace_end_percent=1.0,    # Stop applying VACE at this percentage of steps (1.0 = until end)
                width = 1280,
                height = 720,
        ...
        if vace :
            # Handle both single and multi-VACE inputs
            if multi_vace_inputs is not None and len(multi_vace_inputs) > 0:
                # --- ComfyUI-style Strength Scheduling ---
                frames_list = []
                masks_list = []
                ref_images_list = []
                vace_strength_schedules = []  # Store pre-calculated strength schedule for each stream

                for vace_input in multi_vace_inputs:
                    frames = vace_input.get('frames')
                    masks = vace_input.get('masks') 
                    ref_images = vace_input.get('ref_images')
                    strength = vace_input.get('strength', 1.0)
                    start_percent = vace_input.get('start_percent', vace_start_percent)
                    end_percent = vace_input.get('end_percent', vace_end_percent)
                    
                    if frames is not None:
                        frames = [u.to(self.device) for u in frames]
                    if masks is not None:
                        masks = [u.to(self.device) for u in masks]
                    if ref_images is not None:
                        ref_images = [None if u == None else [v.to(self.device) for v in u] for u in ref_images]
                    
                    frames_list.append(frames)
                    masks_list.append(masks)
                    ref_images_list.append(ref_images)
                    
                    # Pre-calculate the strength for each step
                    schedule = torch.zeros(sampling_steps)
                    start_step = int(start_percent * sampling_steps)
                    end_step = int(end_percent * sampling_steps)
                    schedule[start_step:end_step] = strength
                    vace_strength_schedules.append(schedule)
                
                # Encode all VACE streams
                z0_list = self.vace_encode_frames_multi(frames_list, ref_images_list, masks_list, VAE_tile_size, overlapped_latents)
                m0_list = self.vace_encode_masks_multi(masks_list, ref_images_list)
                z_list = self.vace_latent_multi(z0_list, m0_list)
                
                # Filter out None streams and prepare scheduling
                valid_streams = []
                valid_schedules = []
                for z_stream, schedule in zip(z_list, vace_strength_schedules):
                    if z_stream is not None:
                        valid_streams.append(z_stream)
                        valid_schedules.append(schedule)
                
                z = valid_streams
                vace_strength_schedules = valid_schedules
                # --- End ComfyUI-style Scheduling ---

                if len(z) > 0:
                    target_shape = list(z0_list[0][0].shape) if z0_list[0] is not None else None
                    if target_shape:
                        target_shape[0] = int(target_shape[0] / 2)
                else:
                    # Fallback to single VACE if no valid multi streams
                    if input_frames is not None:
                        ...
                        # Single VACE scheduling (ComfyUI-style)
                        schedule = torch.zeros(sampling_steps)
                        start_step = int(vace_start_percent * sampling_steps)
                        end_step = int(vace_end_percent * sampling_steps)
                        schedule[start_step:end_step] = context_scale
                        vace_strength_schedules = [schedule]
                        z = [z] # Wrap in a list to be consistent

                        target_shape = list(z0[0].shape)
                        target_shape[0] = int(target_shape[0] / 2)
                    else:
                        z = None
                        vace_strength_schedules = []
            else:
                # Single VACE mode (backward compatibility)
                if input_frames is not None:
                    input_frames = [u.to(self.device) for u in input_frames]
                    input_ref_images = [ None if u == None else [v.to(self.device) for v in u]  for u in input_ref_images]
                    input_masks = [u.to(self.device) for u in input_masks]
        ...
        if vace:
            ...
            # The 'vace_contexts' and 'vace_context_scales' kwargs are now the primary way to pass VACE info
            kwargs.update({'vace_contexts': z, 'vace_context_scales': vace_strength_schedules})
            
            if overlapped_latents != None :
                overlapped_latents_size = overlapped_latents.shape[1] + 1
                if isinstance(z, list) and len(z) > 0 and isinstance(z[0], list):
                    # Multi-VACE: use first valid stream for overlapped latents
                    first_valid_z = next((stream for stream in z if stream is not None), None)
                    if first_valid_z:
                        z_reactive = [zz[0:16, 0:overlapped_latents_size + ref_images_count].clone() for zz in first_valid_z]
                else:
                    # Single VACE
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            # --- ComfyUI-style: Look up pre-calculated strengths for the current step ---
            if vace and z is not None and vace_strength_schedules:
                current_vace_strengths = []
                active_contexts = []
                
                for vace_ctx, schedule in zip(z, vace_strength_schedules):
                    strength_for_this_step = schedule[i].item() # Get strength for current step i
                    if strength_for_this_step > 0:
                        active_contexts.append(vace_ctx)
                        current_vace_strengths.append(strength_for_this_step)
                
                # Update kwargs with only the active contexts and their strengths for this specific step
                kwargs.update({'vace_contexts': active_contexts, 'vace_context_scales': current_vace_strengths})
            # --- End ComfyUI-style strength lookup ---
