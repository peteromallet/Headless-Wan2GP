        ...
        if vid2vid_init_video is not None and vid2vid_init_strength < 1.0:
            try:
                ...
                if len(vid2vid_frames) > 0:
                    ...
                    # Adjust timesteps to skip early denoising steps (since we're starting from less noise)
                    # skip_steps = int((1.0 - vid2vid_init_strength) * len(timesteps))
                    # For now, we use full timesteps but the blended latent has less noise
                    # This is simpler and works well with VACE
