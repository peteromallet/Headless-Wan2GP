...
class VaceWanAttentionBlock(WanAttentionBlock):
    ...
    def forward(self, hints, x, **kwargs):
        # This block now receives a list of dicts, each with a 'context' and 'strength'
        if not (isinstance(hints, list) and len(hints) > 0 and isinstance(hints[0], dict)):
            return torch.zeros_like(x) if x is not None else None

        total_skip_connection = None

        for hint_data in hints:
            c = hint_data.get('context')
            strength = hint_data.get('strength', 1.0)

            if c is None or strength == 0.0:
                continue

            # Process this specific context through the attention block
            processed_c = c
            if self.block_id == 0:
                processed_c = self.before_proj(processed_c)
                if x is not None:
                    # Add the main UNet stream (x) only to the first VACE context's processing path
                    # This prevents x from being added multiple times if there are multiple VACE streams
                    if total_skip_connection is None:
                        processed_c += x
            
            processed_c = super().forward(processed_c, **kwargs)
            c_skip = self.after_proj(processed_c)

            # Accumulate the scaled skip connection
            scaled_skip = c_skip * strength
            if total_skip_connection is None:
                total_skip_connection = scaled_skip
            else:
                total_skip_connection += scaled_skip
        
        return total_skip_connection if total_skip_connection is not None else torch.zeros_like(x)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
                 ...
                 max_vace_streams = 4,  # Maximum number of VACE streams to support
                 ):
        r"""
        ...
        if vace_layers != None:            
            ...
            # Additional patch embeddings for multi-VACE (sharing weights with main one by default)
            self.max_vace_streams = max_vace_streams
            self.multi_vace_patch_embeddings = nn.ModuleList([
                nn.Conv3d(self.vace_in_dim, self.dim, kernel_size=self.patch_size, stride=self.patch_size)
                for _ in range(max_vace_streams)
            ])
            
            # Initialize additional embeddings with same weights as main one
            for embedding in self.multi_vace_patch_embeddings:
                embedding.weight.data.copy_(self.vace_patch_embedding.weight.data)
                if embedding.bias is not None and self.vace_patch_embedding.bias is not None:
                    embedding.bias.data.copy_(self.vace_patch_embedding.bias.data)
        if recammaster :
    ...
    def forward(
        ...
        vace_contexts = None,  # List of VACE contexts
        vace_context_scales = None,  # List of strengths for each context
        clip_fea=None,
        y=None,
        freqs = None,
        pipeline = None,
        ...
        # Handle VACE contexts (both single and multiple)
        if vace_contexts is not None and len(vace_contexts) > 0:
            # --- ComfyUI-style: Pass individual contexts and strengths down ---
            hints_per_stream = []
            strengths_per_stream = vace_context_scales if vace_context_scales is not None else [1.0] * len(vace_contexts)

            for i, (vace_ctx, strength) in enumerate(zip(vace_contexts, strengths_per_stream)):
                if vace_ctx is not None and strength > 0.0 and i < len(self.multi_vace_patch_embeddings):
                    # Use dedicated patch embedding for each stream
                    c = [self.multi_vace_patch_embeddings[i](u.to(self.multi_vace_patch_embeddings[i].weight.dtype).unsqueeze(0)) for u in vace_ctx]
                    c = [u.flatten(2).transpose(1, 2) for u in c]
                    c = c[0]
                    hints_per_stream.append({'context': c, 'strength': strength})

            if hints_per_stream:
                 hints_list = [hints_per_stream] * len(x_list)
            else:
            ...
            # Structure as a list of dicts to match the multi-VACE path
            hints_list = [[{'context': c, 'strength': vace_context_scale}]] * len(x_list)
        else:
            hints_list = [None] * len(x_list)
