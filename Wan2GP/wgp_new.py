...
def process_prompt_and_add_tasks(state, model_choice):
    ...
    if mode.startswith("edit_"):
        ...
        if frames_count > max_source_video_frames:
            ...
            # return
        for k in ["image_start", "image_end", "image_refs", "video_guide", "video_guide2", "audio_guide", "audio_guide2", "audio_source" , "video_mask", "video_mask2", "image_mask"]:
            inputs[k] = None    
...
def get_preview_images(inputs):
    inputs_to_query = ["image_start", "image_end", "video_source", "video_guide", "video_guide2", "image_guide", "video_mask", "video_mask2", "image_mask", "image_refs" ]
    labels = ["Start Image", "End Image", "Video Source", "Video Guide", "Video Guide2", "Image Guide", "Video Mask", "Video Mask2", "Image Mask", "Image Reference"]
...
def save_queue_action(state):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for task_index, task in enumerate(queue):
            ...
            video_keys = ["video_guide", "video_mask", "video_guide2", "video_mask2", "video_source", "audio_guide", "audio_guide2", "audio_source"]
...
def prepare_inputs_dict(target, inputs, model_type = None, model_filename = None ):
    ...
    unsaved_params = ["image_start", "image_end", "image_refs", "video_guide", "image_guide", "video_source", "video_mask", "video_guide2", "video_mask2", "image_mask", "audio_guide", "audio_guide2", "audio_source"]
...
def refresh_video_prompt_type_video_guide(state, video_prompt_type, video_prompt_type_video_guide,  image_mode):
    ...
    # Check if multiple processing types are selected for secondary components
    process_letters = ''.join(c for c in video_prompt_type if c in "PDSLCMU")
    secondary_visible = visible and not image_outputs and len(process_letters) > 1
    
    return video_prompt_type,  gr.update(visible = visible and not image_outputs), gr.update(visible = visible and image_outputs), gr.update(visible = visible and not image_outputs), gr.update(visible = visible and "G" in video_prompt_type), gr.update(visible= (visible or "F" in video_prompt_type or "K" in video_prompt_type) and vace), gr.update(visible= visible and not "U" in video_prompt_type ), gr.update(visible= mask_visible and not image_outputs), gr.update(visible= mask_visible and image_outputs), gr.update(visible= mask_visible), gr.update(visible= secondary_visible), gr.update(visible= secondary_visible), gr.update(visible= vace and secondary_visible)
...
def generate_video_tab(update_form = False, state_dict = None, ui_defaults = None, model_family = None, model_choice = None, header = None, main = None):
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Column(visible= vace or phantom or hunyuan_video_custom or hunyuan_video_avatar or hunyuan_video_custom_edit or t2v or ltxv or flux and model_reference_image) as video_prompt_column: 
                ...
                # Secondary video guide and mask for dual preprocessing (visible when multiple processing types are selected)
                def has_multiple_processes(video_prompt_type_value):
                    if not video_prompt_type_value or "V" not in video_prompt_type_value:
                        return False
                    process_letters = ''.join(c for c in video_prompt_type_value if c in "PDSLCMU")
                    return len(process_letters) > 1
                
                video_guide2 = gr.Video(label= "Secondary Control Video (for dual preprocessing)", visible= has_multiple_processes(video_prompt_type_value), value= ui_defaults.get("video_guide2", None))
                video_mask2 = gr.Video(label= "Secondary Video Mask (for dual preprocessing)", visible= has_multiple_processes(video_prompt_type_value), value= ui_defaults.get("video_mask2", None))
            ...
            with gr.Tabs(visible=advanced_ui) as advanced_row:
                ...
                with gr.Tab("General"):
                    with gr.Column():
                        ...
                        with gr.Row(visible = vace) as control_net_weights_row:
                            ...
                            control_net_weight2 = gr.Slider(0.0, 2.0, value=ui_defaults.get("control_net_weight2",1), step=0.1, label="Control Net Weight #2", visible=vace and has_multiple_processes(video_prompt_type_value))
                ...
                with gr.Tab("Quality", visible = vace and image_outputs or any_skip_layer_guidance or any_cfg_zero or any_cfg_star or any_apg ) as quality_tab:
                        ...
                        with gr.Column(visible= any_apg ) as apg_col:
                            gr.Markdown("<B>Correct Progressive Color Saturation during long Video Generations</B>")
                        ...
                        with gr.Column(visible = any_cfg_star) as cfg_free_guidance_col:
                            gr.Markdown("<B>Classifier-Free Guidance Zero Star, better adherence to Text Prompt</B>")
        ...
        if update_form:
            ...
            video_prompt_type_video_guide.input(fn=refresh_video_prompt_type_video_guide, inputs = [state, video_prompt_type, video_prompt_type_video_guide, image_mode], outputs = [video_prompt_type, video_guide, image_guide, keep_frames_video_guide, denoising_strength, video_guide_outpainting_col, video_prompt_type_video_mask, video_mask, image_mask, mask_expand, video_guide2, video_mask2, control_net_weight2])
    ...
    return ( state, loras_choices, lset_name, resolution,
             video_guide, image_guide, video_mask, video_guide2, video_mask2, image_mask, image_refs, prompt_enhancer_row, audio_tab, PP_MMAudio_col  
            ) 
            ...
            with gr.Tab("Video Generator", id="video_gen") as video_generator_tab:
                ...
                with gr.Row():
                    (   state, loras_choices, lset_name, resolution,
                        video_guide, image_guide, video_mask, video_guide2, video_mask2, image_mask, image_refs, prompt_enhancer_row, mmaudio_tab, PP_MMAudio_col
