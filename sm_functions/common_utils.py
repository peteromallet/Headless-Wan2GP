...
def add_task_to_db(task_payload: dict, db_path: str | Path):
    conn = sqlite3.connect(str(db_path))
    cursor = conn.cursor()
    try:
        cursor.execute(
            f"INSERT INTO {DEFAULT_DB_TABLE_NAME} (task_id, params, status) VALUES (?, ?, ?)",
            (task_payload["task_id"], json.dumps(task_payload), STATUS_QUEUED)
        )
        conn.commit()
        print(f"Task {task_payload['task_id']} added to database {db_path}.")
    except sqlite3.Error as e:
        print(f"SQLite error when adding task {task_payload['task_id']}: {e}")
        raise
    finally:
        conn.close()
