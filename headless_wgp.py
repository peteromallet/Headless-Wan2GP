...
class WanOrchestrator:
    ...
    def _resolve_parameters(self, model_type: str, task_params: dict) -> dict:
        ...
        try:
            ...
            dprint(f"[PARAM_DEBUG] get_default_settings('{model_type}') returned: {model_defaults}")
            dprint(f"[PARAM_DEBUG] Type: {type(model_defaults)}")
            
            if model_defaults:
                dprint(f"[PARAM_DEBUG] Before applying model config - resolved_params: {resolved_params}")
                for param, value in model_defaults.items():
                    ...
                    if param not in ["prompt", "activated_loras", "loras_multipliers"]:
                        ...
                        dprint(f"[PARAM_DEBUG] Applied {param}: {old_value} → {value}")
                        
                dprint(f"[PARAM_DEBUG] After applying model config - resolved_params: {resolved_params}")
            ...
            dprint(f"[PARAM_DEBUG] Exception details: {str(e)}")
            import traceback
            dprint(f"[PARAM_DEBUG] Traceback: {traceback.format_exc()}")
        
        # 3. Apply task explicit parameters (highest priority)
        dprint(f"[PARAM_DEBUG] Task explicit parameters: {task_params}")
        dprint(f"[PARAM_DEBUG] Before applying task params - resolved_params: {resolved_params}")
        
        for param, value in task_params.items():
            if value is not None:  # Don't override with None values
                old_value = resolved_params.get(param, "NOT_SET")
                resolved_params[param] = value
                dprint(f"[PARAM_DEBUG] Task override {param}: {old_value} → {value}")
                
        dprint(f"[PARAM_DEBUG] FINAL resolved_params: {resolved_params}")
