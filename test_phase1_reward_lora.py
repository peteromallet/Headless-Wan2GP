#!/usr/bin/env python3
"""Test reward LoRA strength variations on Phase 1 only"""
...
def create_temp_config(reward_strength):
    """Create a temporary config with specific reward LoRA strength for Phase 1 only"""

    config = {
        "model": {
            "name": f"Wan2.2 Vace Fun Cocktail Lightning 3Phase - Reward {reward_strength}",
    ...
    # Copy to the defaults directory with a unique name
    config_name = f"vace_fun_14B_cocktail_lightning_3phase_reward_{str(reward_strength).replace('.', '_')}"
    config_path = Path("Wan2GP/defaults") / f"{config_name}.json"
    shutil.copy2(temp_config.name, config_path)
...
def test_phase1_reward_lora():
    ...
    # Test configurations - reward LoRA strengths for Phase 1 (highest first)
    ...
    print("🚀 Phase 1 Reward LoRA Strength Tests")
    print("=" * 60)
    print(f"Testing {len(reward_strengths)} different reward LoRA strengths on Phase 1 ONLY:")
    for i, strength in enumerate(reward_strengths, 1):
    ...
    try:
        for i, reward_strength in enumerate(reward_strengths, 1):
            print(f"\n--- Test {i}/{len(reward_strengths)}: Reward LoRA strength {reward_strength} (Phase 1 ONLY) ---")

            # Create temporary config for this reward strength
            config_name, config_path = create_temp_config(reward_strength)
        ...
        for config_path in temp_configs:
            try:
                config_path.unlink()
                print(f"🧹 Cleaned up temp config: {config_path.name}")
            except:
                pass
...
def main():
    ...
    print("Testing reward LoRA at 1.0, 0.75, 0.5, 0.25 strength on Phase 1 ONLY")
